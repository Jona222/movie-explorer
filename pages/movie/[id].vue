...
useHead({
  ...
  meta: [
    {name: 'description', content: computed(() => movie.value?.overview || 'Movie details page')},
    {property: 'og:title', content: computed(() => movie.value?.title || 'Movie Explorer')},
    {property: 'og:description', content: computed(() => movie.value?.overview || 'Movie details page')},
...
const shareMovie = () => {
  ...
  navigator.share ? navigator.share(shareData).catch(console.error) : alert('Sharing not supported.');
};
</script>
